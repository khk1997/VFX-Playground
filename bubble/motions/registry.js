...
export const MOTIONS = {
  ...
  research: {
    ...
    // 讀成高光的反射，在淺底是暗斑。
    lightOverrides: {
      // 淺底使用獨立的吸收、反射與後處理預設，其餘沿用深底的定案值 ——
      // 這個模式在白底上要看起來就是「同一個材質換了背景」，所以材質本身
      // 的參數（IOR、粗糙度、色散、焦散、稜光）刻意一根都不動。
      //
      // 體積吸收：深底時它是無作用項（折射進來的背景是黑的，乘什麼都是黑），
      // 所以那邊給到 5.05 不痛不癢；白底時它變成身體唯一的填充光在被衰減，
      // 5.05 會把中央壓成灰褐色，那就是原本「黑黑髒髒」的來源。
      absorb: 2.6,
      // 反射要往上加才追得回輪廓：棚燈 HDRI 的平均亮度低於白紙，同一道在深底
      // 讀成高光的反射，在白底是暗斑。
      reflect: 1.7,
      postExposure: 1.0,
      postBrightness: 0.0,
      // 白棚玻璃保留乾淨透射與細高光；深底的顆粒設定獨立保留。
      postGrain: 0.0,
    },
    params: [
      {
