...
export default function createShapeRigidMotion(P) {
  function shapeRigidMotion(phase) {
    ...
    const cycles = Math.max(1, Math.round(P.shapeMotionCycles));
    const base = phase * cycles * Math.PI * 2;
    // 疊加二次諧波做出不對稱的「蓄力→回彈」波形；ease 是疊加比例，
    // ease=0 時退化回單純正弦，跟加這個參數之前的行為完全等價。
    const ease = Math.max(0, Math.min(1, P.shapeMotionEase));
    const h2 = ease * 0.35;
    ...
    const angleX = spin * P.shapeSpinX * Math.PI / 180;
    const angleY = spin * P.shapeSpinY * Math.PI / 180;
    const angleZ = spin * P.shapeSpinZ * Math.PI / 180;

    const breatheAngle = base + Math.PI * 2 / 3;
    const breathe = 1 + wave(breatheAngle) * P.shapeBreathe;

    const bobAngle = base + Math.PI * 4 / 3;
    const offsetY = wave(bobAngle) * P.shapeBob;
    ...
    const stretch = bobVel * P.shapeSquash;
